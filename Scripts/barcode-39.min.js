...
(function () {
    ...
    ["%", "NnNwNwNwN", "bwbWbWbWb"], ["*", "NwNnWnWnN", "bWbwBwBwb"]], l = function (b, c) { b = ("" + b).toUpperCase(); c = c ? +c : 2; for (var a = 0; a < h.length; a++)if (b === h[a][0]) return h[a][c] }, k = []; d = function (b, c, a) { this.codes = h; this.version = "0.0.2"; this.barcode = null; this.element = document.getElementById(b || "barcode"); this.delimeter = a || "*"; this.type = c || 2; this.init() }; d.prototype.toString = function () { return this.barcode ? (this.delimeter + this.barcode + this.delimeter).trim() : "" }; d.prototype.toBarcode = function () {
        var b = k.join(""),
        b = b.slice(0, b.length - 1); return b.trim()
    }; d.prototype.toDataURL = function () {
        if ("undefined" === typeof this.element) throw Error("Element not defined."); var b = document.createElement("canvas"); if (!b.getContext) throw Error("Canvas not supported."); var c = this.element.innerText.toUpperCase().trim(), a = +c.length, d = 15 * a + (a - 1), f = 360 > a ? 300 : a + 40, a = b.getContext("2d"); a.clearRect(0, 0, b.width, b.height); b.height = 50; b.width = f; a.fillStyle = "rgba(255,255,255, 0.0)"; a.fillRect(0, 0, b.width, b.height); for (var e = Math.round(f /
            2 - d / 2), d = 0; d < c.length; d++) { var g = l(c[d]); k.push("[" + g + "]w"); if ("" === typeof g || "undefined" === typeof g) g = l("-"); for (f = 0; f < g.length; f++)a.fillStyle = 0 == f % 2 ? "rgb(0,0,0)" : "rgba(255,255,255, 0.0)", 91 > g.charCodeAt(f) ? (a.fillRect(e, 5, 3, 50), e += 3) : (a.fillRect(e, 5, 1, 50), e += 1); d != c.length - 1 && (a.fillStyle = "rgba(255,255,255, 0.0)", a.fillRect(e, 5, 1, 50), e += 1) } this.barcode = c; return b.toDataURL()
    }; d.prototype.init = function () { k = []; var b = this.toDataURL(); this.element.innerHTML = '<img src="' + b + '" />' }; window.Barcode39 =
        d
})();